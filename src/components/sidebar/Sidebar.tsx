import { Home, Plus, Search, Settings, Star, Trash2, X } from "lucide-react";
...
function Sidebar() {
  ...
  const { setCommandOpen, setTrashOpen, setSettingsOpen, sidebarOpen, setSidebarOpen } =
    useUIStore();
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-full w-64 -translate-x-full flex-col border-r border-zinc-800 bg-zinc-900 transition-transform duration-200 md:static md:z-auto md:translate-x-0 ${
      ...
      >
        ...
        <div className="space-y-0.5 p-2">
          <button
            onClick={() => {
              if (rootPages[0]) selectPage(rootPages[0].id);
